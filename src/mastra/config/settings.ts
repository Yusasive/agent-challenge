  ...
  const rawConfig = {
    ...
    timeouts: {
      request: process.env.REQUEST_TIMEOUT
        ...
        : undefined,
      model: process.env.MODEL_TIMEOUT
        ? parseInt(process.env.MODEL_TIMEOUT, 10)
        : undefined,
      analysis: process.env.ANALYSIS_TIMEOUT
        ? parseInt(process.env.ANALYSIS_TIMEOUT, 10)
        : undefined,
      verification: process.env.VERIFICATION_TIMEOUT
        ? parseInt(process.env.VERIFICATION_TIMEOUT, 10)
        : undefined,
    },
    ...
    security: {
      ...
      maxRequestsPerMinute: process.env.MAX_REQUESTS_PER_MINUTE
        ...
        : undefined,
      maxContractSize: process.env.MAX_CONTRACT_SIZE
        ? parseInt(process.env.MAX_CONTRACT_SIZE, 10)
        : undefined,
    ...
    analysis: {
      maxAnalysisDepth: process.env.MAX_ANALYSIS_DEPTH as
        | "basic"
        | "intermediate"
        | "deep",
      ...
      enableFormalVerification:
        process.env.ENABLE_FORMAL_VERIFICATION !== "false",
    ...
    logging: {
      ...
      maxLogFiles: process.env.MAX_LOG_FILES
        ? parseInt(process.env.MAX_LOG_FILES, 10)
        : undefined,
    },

    performance: {
      maxConcurrentAnalysis: process.env.MAX_CONCURRENT_ANALYSIS
        ? parseInt(process.env.MAX_CONCURRENT_ANALYSIS, 10)
        : undefined,
      enableCaching: process.env.ENABLE_CACHING !== "false",
      cacheTimeout: process.env.CACHE_TIMEOUT
        ? parseInt(process.env.CACHE_TIMEOUT, 10)
        : undefined,
    },
  };
...
export const validateConfig = () => {
  ...
  console.log(
    `  - Rate Limiting: ${security.enableRateLimiting ? "Enabled" : "Disabled"}`
  );
  console.log(
    `  - Advanced Analysis: ${analysis.enableAdvancedAnalysis ? "Enabled" : "Disabled"}`
  );
  console.log(
    `  - ML Detection: ${analysis.enableMLDetection ? "Enabled" : "Disabled"}`
  );
  console.log(
    `  - Formal Verification: ${analysis.enableFormalVerification ? "Enabled" : "Disabled"}`
  );
