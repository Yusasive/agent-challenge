...
export const accessibilityEnhancer = createTool({
  ...
  description: "Makes audit results accessible to non-technical users with simplified explanations and visual summaries",
  inputSchema: z.object({
    ...
    targetAudience: z.enum(['developer', 'business', 'general']).optional().default('general'),
    includeVisuals: z.boolean().optional().default(true),
    simplificationLevel: z.enum(['basic', 'intermediate', 'detailed']).optional().default('basic'),
  }),
  outputSchema: z.object({
    ...
    developerFriendly: z.object({
      ...
      securityChecklist: z.array(z.object({
        item: z.string(),
...
const generateSimplifiedSummary = (auditResults: any): AccessibilityReport['simplifiedSummary'] => {
  const securityScore = auditResults.securityScore || 0;
  const issues = auditResults.issues || [];
  const vulnerabilities = auditResults.vulnerabilities || [];
  
  const criticalIssues = [...issues, ...vulnerabilities].filter(item => 
    item.severity === 'Critical'
  );
  const highIssues = [...issues, ...vulnerabilities].filter(item => 
    item.severity === 'High'
  );
  ...
  const mainConcerns = [
    ...
    ...highIssues.slice(0, 2).map(issue => 
...
const generateDeveloperFriendlyReport = (auditResults: any): AccessibilityReport['developerFriendly'] => {
  const issues = auditResults.issues || [];
  ...
  const bestPractices = {
    ...
    missing: issues.filter((issue: any) => 
  ...
  const securityChecklist = [
    {
      ...
      status: issues.some((i: any) => i.type?.includes('Reentrancy')) ? 'Fail' : 'Pass',
      explanation: 'Protects against recursive call attacks',
    },
    {
      item: 'Access Control',
      status: issues.some((i: any) => i.type?.includes('Access')) ? 'Warning' : 'Pass',
      explanation: 'Ensures only authorized users can call sensitive functions',
    },
    {
      item: 'Integer Overflow Protection',
      status: issues.some((i: any) => i.type?.includes('Overflow')) ? 'Fail' : 'Pass',
...
const generateBusinessImpactReport = (auditResults: any): AccessibilityReport['businessImpact'] => {
  const securityScore = auditResults.securityScore || 0;
  const issues = auditResults.issues || [];
  ...
  if (criticalCount > 0) {
    ...
    riskToBusiness = 'CRITICAL - Potential for significant financial loss and reputation damage';
    ...
    riskToBusiness = 'MODERATE - Potential for security incidents and user complaints';
  ...
  const recommendedActions = [
    deploymentReadiness === 'Not Ready' ? 'Halt deployment immediately' : 'Proceed with caution',
...
const generateVisualSummary = (auditResults: any): AccessibilityReport['visualSummary'] => {
  const securityScore = auditResults.securityScore || 0;
  ...
  const categories = {
    ...
    'Code Quality': {
      ...
      status: securityScore >= 70 ? 'Good' : securityScore >= 50 ? 'Fair' : 'Poor',
    },
    'Gas Efficiency': {
      score: Math.max(60, securityScore - 10),
      status: securityScore >= 75 ? 'Good' : securityScore >= 55 ? 'Fair' : 'Poor',
    },
    'Best Practices': {
      score: Math.max(50, securityScore - 5),
      status: securityScore >= 80 ? 'Good' : securityScore >= 65 ? 'Fair' : 'Poor',
    },
  } as { [category: string]: { score: number; status: 'Good' | 'Fair' | 'Poor' } };

  return {
    riskMeter: 100 - securityScore, // Inverted for risk
    securityGrade,
    categories,
  };
};
...
const generateAccessibleMarkdownReport = (
  ...
  return `# 🛡️ Smart Contract Security Report
...
**Deployment Status:** ${businessImpact.deploymentReadiness === 'Ready' ? '✅ Ready' : businessImpact.deploymentReadiness === 'Needs Work' ? '⚠️ Needs Work' : '❌ Not Ready'}
...
### Security Checklist
${developerFriendly.securityChecklist.map((item: any) => 
...
${Object.entries(visualSummary.categories).map(([category, data]: [string, any]) => 
