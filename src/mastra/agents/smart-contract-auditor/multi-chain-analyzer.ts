...
export const multiChainAnalyzer = createTool({
  ...
  description:
    "Analyzes smart contracts for multi-chain compatibility and chain-specific vulnerabilities",
  inputSchema: z.object({
    ...
    targetChains: z
      ...
      .optional()
      .describe(
        "Specific chains to analyze for (ethereum, bsc, polygon, arbitrum, optimism)"
      ),
...
const analyzeMultiChain = async (
  ...
  const chainSpecificIssues = analyzeChainSpecificIssues(
    contractCode,
    targetChains,
    chainConfigs
  );
...
const detectChain = (
  ...
  if (
    contractCode.includes("IERC20") ||
    contractCode.includes("OpenZeppelin")
  ) {
    return "ethereum";
  }
  ...
  targetChains.forEach((chain) => {
    ...
    if (chain === "ethereum") {
      if (contractCode.includes("for (") && contractCode.includes("++")) {
        issues.push({
          ...
          recommendation:
            "Consider batch operations or move to L2 for gas-intensive operations",
        });
      }

      if (
        contractCode.includes("block.timestamp") &&
        contractCode.includes("deadline")
      ) {
        issues.push({
          chain: "ethereum",
          ...
          severity: "Medium",
          description:
            "Timestamp-based logic vulnerable to MEV attacks on Ethereum",
    ...
    if (chain === "bsc") {
      if (
        ...
        !contractCode.includes("Ownable2Step")
      ) {
        issues.push({
          chain: "bsc",
          ...
          severity: "Medium",
          description:
            "BSC has fewer validators, making centralization risks more critical",
          recommendation:
            "Implement multi-sig or DAO governance instead of single owner",
        });
      }
    }
    ...
    if (chain === "polygon") {
      if (
        contractCode.includes("withdraw") &&
        !contractCode.includes("checkpoint")
      ) {
        issues.push({
          chain: "polygon",
          ...
          severity: "Medium",
          description:
            "Polygon withdrawals depend on Ethereum checkpoint submissions",
    ...
    if (chain === "arbitrum") {
      if (contractCode.includes("block.number")) {
        issues.push({
          ...
          recommendation:
            "Use block.timestamp or Arbitrum-specific block tracking",
        });
      }
    }
    ...
    if (chain === "optimism") {
      if (contractCode.includes("selfdestruct")) {
        issues.push({
          ...
          recommendation:
            "Avoid selfdestruct or implement Optimism-specific handling",
        });
      }
    }
  });
...
const analyzeCrossChainCompatibility = (
  ...
  if (
    contractCode.includes("chainid") ||
    contractCode.includes("block.chainid")
  ) {
    issues.push("Contract uses chain ID which will differ across chains");
    recommendations.push(
      "Make chain ID configurable or use chain-agnostic logic"
    );
  }
  ...
  if (addresses && addresses.length > 0) {
    issues.push(
      "Contract contains hardcoded addresses that may not exist on all chains"
    );
    recommendations.push(
      "Use factory patterns or configurable addresses for cross-chain deployment"
    );
  }
  ...
  if (contractCode.includes("gasleft()") || contractCode.includes("gas:")) {
    issues.push(
      "Contract makes gas assumptions that may not hold across different chains"
    );
    recommendations.push(
      "Make gas limits configurable or use chain-specific gas strategies"
    );
  }
